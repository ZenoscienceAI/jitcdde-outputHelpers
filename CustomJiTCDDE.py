...
class customjitcdde(jitcdde):
    ...
    attr:
        - Nhelpers (int): The number of helpers
        - helperNames (list[str]): The names of the helpers
    """
    def __init__(self, *args, helpers = None, **kwargs):
        """Initializes 'customjitcdde'
        args:
            - args (tuple): unnamed arguments
            - helpers (list[tuple[sympy.Symbol, sympy.Expression]]): Helper equations
        """
    ...
    def integrate(self, target_time):
        ...
        if self.DDE.get_t() > target_time:
            warn(
                ...
                "should think about increasing your sampling time).", stacklevel=2
                )
